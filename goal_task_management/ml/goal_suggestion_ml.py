...
def validate_model_output_size():
    ...
    output_size = (
        get_output_size()
    )  # This should be the number of classes the model was trained on

    if current_goal_count != output_size:
        print(
            f"Number of goals has changed from {output_size} to {current_goal_count}. Retraining the model."
        )
...
def train_pytorch_model(model_path=MODEL_PATH):
    ...
    model = GoalPredictionModel(
        input_size=X.shape[1], hidden_size=128, output_size=len(unique_goals)
    )
...
def preprocess_user_data(user_profile, role):
    ...
    padding_needed = max(
        0, expected_input_size - 2 - len(roles)
    )  # Subtract 2 for age and gender
