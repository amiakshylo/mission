...
class GoalSuggestionsViewset(CreateModelMixin, GenericViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        print(
            f"User Profile: {user_profile}, Roles: {[role.title for role in roles_queryset]}"
        )
        ...
        return Response(
            suggestion_serializer.errors, status=status.HTTP_400_BAD_REQUEST
        )
    ...
    def suggest_goals(self, user_profile, role, top_n=5):
        ...
        if unique_goals_count < 50:  # Replace 50 with your desired threshold
            print(
                f"Less than 50 unique goals for role '{role.title}'. Generating goals with AI."
            )
            while unique_goals_count < 50:
                generated_goals = generate_goal_with_openai(
                    user_profile, selected_role=role.title
                )
                if generated_goals:
                    # Save only unique goals
                    new_goals = self.save_generated_goals(
                        user_profile, generated_goals, role
                    )
            ...
            with torch.no_grad():
                ...
                for idx_tensor in top_n_indices[0]:
                    ...
                    if idx < Goal.objects.count() and idx not in used_indices:
                        ...
                        goal = Goal.objects.filter(
                            id=predicted_goal_id, role=role
                        ).first()
                        if goal:
                        ...
                        print(
                            f"Predicted index {idx} is out of bounds for current goal count."
                        )
                ...
                if len(suggested_goals) < top_n:
                    additional_goals = Goal.objects.filter(role=role).exclude(
                        id__in=[goal.id for goal in suggested_goals]
                    )[: top_n - len(suggested_goals)]
    ...
    def save_generated_goals(
        self, user_profile, generated_goals, role, source="openai"
    ):
        created_goals = []
        logged_goals = []
        ...
        for goal_data in generated_goals:
            ...
            for existing_goal in existing_goals:
                existing_lemmatized_title = lemmatize_title(
                    normalize_text(existing_goal.title)
                )

                if lemmatized_title == existing_lemmatized_title or are_titles_similar(
                    lemmatized_title, existing_lemmatized_title
                ):
